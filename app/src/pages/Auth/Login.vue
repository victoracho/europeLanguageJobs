  ...
  <div class="login-box">
    ...
    <div class="card">
      <div class="card-body login-card-body rounded">
        <p class="login-box-msg" id="view-login-msg">
          ...
          <div v-if="!haveLoginSaved || needLoginManual">
            <form id="form-login" @submit.prevent="submitLogin">
              ...
              <div class="input-group mb-3">
                ...
                />
                <div class="input-group-append">
                  <div class="input-group-text" @click="passwordWatch(true)">
                    <span class="fas fa-lock" id="span-password"></span>
                  </div>
                </div>
              </div>
              <div class="row mb-3">
  ...
  methods: {
    async submitLogin() {
      ...
      if (this.thisEmail && this.thisPassword) {
        ...
        if (mail.result) {
          ...
          await this.postLogin(this.thisEmail, this.thisPassword);
        } else {
      ...
      this.$axios
        ...
        .then(() => {
          this.$axios
            .postLogin(
              email,
              ForgeJs.encryptPassword(password),
              this.$Device("login", true)
            )
    ...
    async loginResponse(data) {
      ...
      if (data.status === "success") {
        ...
        await this.$SavedLogin.loginSave({
          name: data.response_data.account_name,
          profile_img: data.response_data.account_img,
          username: this.thisEmail,
    ...
    catchError(error) {
      this.$("#view-login-msg").html(
        this.spanMessage("danger", this.$axiosRes.catchError(error))
      );
      this.$("#input-submit").prop("disabled", false);
